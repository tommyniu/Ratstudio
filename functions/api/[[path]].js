async function handleRequest(request, env) {
  ...
  try {
    ...
    if (act === "createComment") {
      ...
      const user = db.users.find(u => u.uid == uid);
      if (!user || !pid || !text) return new Response("error", { headers: corsHeaders });
    ...
    if (act === "likeComment") {
      const c = db.comments.find(x => x.cid == url.searchParams.get("cid"));
      if (c) c.like++;
      await safeWrite("db", JSON.stringify(db));
      return new Response("ok", { headers: corsHeaders });
    }
    ...
    if (url.pathname === "/api/reg") {
      ...
      await safeWrite("db", JSON.stringify(db));
      return new Response(`uid:${nu}`, { headers: corsHeaders });
    }
    ...
    if (url.pathname === "/api/userStat") {
      ...
      const ps = db.posts.filter(p => p.uid == u);
      const likes = ps.reduce((s, i) => s + (i.like || 0), 0);
      return new Response(JSON.stringify({ post: ps.length, like: likes }), {
        headers: { ...corsHeaders, "Content-Type": "application/json" }
      });
    }
